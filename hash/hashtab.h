...
* The table length splits when the table length equals the number of items
  ...
  hfindInt - find an item keyed by an integer in the table
   hadd    - insert an item into the table
  haddInt  - insert an item into the table with an integer key
...
/* celhcreate - create a hash table
   ...
   RETURNS:
     the new table
 */
htab *celhcreate(/*_ word logsize _*/);
...
/* hfindInt - move the current position to a given Integer key
   ARGUMENTS:
     t    - the hash table
     key  - the key to look for
   RETURNS:
     ...
     If the item exists, moves the current position to that item.
 */
word  hfindInt(/*_ htab *t, ub4 key _*/);
...
/* haddInt - add a new item to the hash table
          change the position to point at the item with the key
   ARGUMENTS:
     t     - the hash table
     key   - the key to look for
   ...
   RETURNS:
     FALSE if the operation fails (because that key is already there).
 */
word  haddInt(/*_ htab *t, ub4 key, void *stuff _*/);
