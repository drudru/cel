...
* The table length splits when the table length equals the number of items
  ...
  celhcreate  - create a hash table
  celhdestroy - destroy a hash table
  ...
  haddInt  - insert an integer keyed itme into the table
  hfindInt - find a slot based on an integer key
--------------------------------------------------------------------
*/
...
htab  *t;    /* table */
{
  ...
  register hitem **newtab = (hitem **)htmalloc(newsize*sizeof(hitem *));
...
/* celhcreate - create a hash table initially of size power(2,logsize) */
htab *celhcreate(logsize)
word  logsize;    /* log base 2 of the size of the hash table */
{
  ub4 i,len;
  htab *t = (htab *)htmalloc(sizeof(htab));

  len = ((ub4)1<<logsize);
  t->table = (hitem **)htmalloc(sizeof(hitem *)*(ub4)len);
...
/* celhdestroy - destroy the hash table and free all its memory */
void celhdestroy( t)
htab  *t;    /* the table */
{
//  hitem *h;
...
/* hfindInt - find an item with a given key in a hash table */
word   hfindInt( t, key )
htab  *t;     /* table */
ub4    key;   /* key to find */
{
  ub4    keyl = 4;  /* key length */
  hitem *h;
  ub4    x = lookup(&key,4,0);
  ub4    y;
  for (h = t->table[y=(x&t->mask)]; h; h = h->next)
  {
    if ((x == h->hval) && 
...
/*
 * haddInt - add an item to a hash table keyed by an integer
 * return FALSE if the key is already there, otherwise TRUE.
 */
word haddInt ( t, key, stuff)
htab  *t;      /* table */
ub4    key;    /* key to add to hash table */
void  *stuff;  /* stuff to associate with this key */
{
  register hitem  *h,**hp;
  register ub4     y, x = lookup(&key,4,0);
  ub4      keyl    = 4;
